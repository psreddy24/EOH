...
export class EmployeeService {
  ...
  protected convertDateFromClient(employee: IEmployee): IEmployee {
    const copy: IEmployee = Object.assign({}, employee, {
      employedDate: employee.employedDate && employee.employedDate.isValid() ? employee.employedDate.toJSON() : undefined,
      terminatedDate: employee.terminatedDate && employee.terminatedDate.isValid() ? employee.terminatedDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.employedDate = res.body.employedDate ? moment(res.body.employedDate) : undefined;
      res.body.terminatedDate = res.body.terminatedDate ? moment(res.body.terminatedDate) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((employee: IEmployee) => {
        employee.employedDate = employee.employedDate ? moment(employee.employedDate) : undefined;
        employee.terminatedDate = employee.terminatedDate ? moment(employee.terminatedDate) : undefined;
      });
    }
    return res;
  }
